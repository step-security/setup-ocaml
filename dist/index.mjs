import { A as info, C as WINDOWS_ENVIRONMENT, D as error, E as debug, F as __require, I as __toESM, M as exec, N as __commonJSMin, O as exportVariable, P as __exportAll, S as PLATFORM, T as addPath, _ as DUNE_CACHE_ROOT, b as OPAM_REPOSITORIES, c as installOcaml, d as repositoryRemoveAll, f as setupOpam, g as DUNE_CACHE, h as CYGWIN_ROOT_BIN, i as restoreOpamCache, j as isDebug, k as group, l as pin, m as CYGWIN_BASH_ENV, o as saveOpamCache, p as update, r as restoreDuneCache, s as resolvedCompiler, t as installDune, u as repositoryAddAll, v as OPAM_LOCAL_PACKAGES, w as require_src$1, x as OPAM_ROOT, y as OPAM_PIN } from "./dune.mjs";
...
function ensureAbsoluteRoot(root, itemPath) {
	...
	if (root.endsWith("/") || IS_WINDOWS$4 && root.endsWith("\\")) {} else root += path$3.sep;
...
function safeTrimTrailingSeparator(p) {
	...
	if (!p.endsWith(path$3.sep)) return p;
	if (p === path$3.sep) return p;
...
/* c8 ignore stop */
const sep = defaultPlatform === "win32" ? path$2.win32.sep : path$2.posix.sep;
minimatch.sep = sep;
	...
	*/
	constructor(itemPath) {
		...
		if (typeof itemPath === "string") {
			...
			if (!hasRoot(itemPath)) this.segments = itemPath.split(path$3.sep);
			else {
			...
			for (let i = 0; i < itemPath.length; i++) {
				...
				if (i === 0 && hasRoot(segment)) {
					...
					assert(!segment.includes(path$3.sep), `Parameter 'itemPath' contains unexpected path separators`);
					this.segments.push(segment);
				}
			}
		}
...
var Pattern = class Pattern {
	constructor(patternOrNegate, isImplicitPattern = false, segments, homedir) {
		...
		*/
		...
		this.trailingSeparator = normalizeSeparators(pattern).endsWith(path$3.sep);
	...
	*/
	match(itemPath) {
		if (this.segments[this.segments.length - 1] === "**") {
			itemPath = normalizeSeparators(itemPath);
			if (!itemPath.endsWith(path$3.sep) && this.isImplicitPattern === false) itemPath = `${itemPath}${path$3.sep}`;
	...
	*/
	static fixupPattern(pattern, homedir) {
		...
		if (pattern === "." || pattern.startsWith(`.${path$3.sep}`)) pattern = Pattern.globEscape(process.cwd()) + pattern.substr(1);
		else if (pattern === "~" || pattern.startsWith(`~${path$3.sep}`)) {
	...
	globGenerator() {
		return __asyncGenerator(this, arguments, function* globGenerator_1() {
			...
			for (const searchPath of getSearchPaths(patterns)) {
				...
				try {
					yield __await(fs$3.promises.lstat(searchPath));
				} catch (err) {
			...
			while (stack.length) {
				...
				if (!stats) continue;
				if (options.excludeHiddenFiles && path$3.basename(item.path).match(/^\./)) continue;
				if (stats.isDirectory()) {
					...
					const childItems = (yield __await(fs$3.promises.readdir(item.path))).map((x) => new SearchState(path$3.join(item.path, x), childLevel));
	...
	static stat(item, options, traversalChain) {
		return __awaiter$2(this, void 0, void 0, function* () {
			...
			if (options.followSymbolicLinks) try {
				stats = yield fs$3.promises.stat(item.path);
			} catch (err) {
				if (err.code === "ENOENT") {
			...
			else stats = yield fs$3.promises.lstat(item.path);
			if (stats.isDirectory() && options.followSymbolicLinks) {
				const realPath = yield fs$3.promises.realpath(item.path);
...
async function installer() {
	...
	if (OPAM_PIN) {
		const fnames = await (await create$1(OPAM_LOCAL_PACKAGES)).glob();
		await pin(fnames);
	}
	...
	return wrapAsync(async function dispatchHttpRequest(resolve$1, reject, onDone) {
		...
		if (protocol === "data:") {
			if (maxContentLength > -1) {
				...
				if (estimateDataURLBufferAllocation(dataUrl) > maxContentLength) return reject(new AxiosError$1("maxContentLength size of " + maxContentLength + " exceeded", AxiosError$1.ERR_BAD_RESPONSE, config));
			}
			let convertedData;
...
async function validateSubscription() {
	const eventPath = process$2.env.GITHUB_EVENT_PATH;
	let repoPrivate;
	if (eventPath && fs.existsSync(eventPath)) repoPrivate = JSON.parse(fs.readFileSync(eventPath, "utf8"))?.repository?.private;
	const upstream = "ocaml/setup-ocaml";
	const action = process$2.env.GITHUB_ACTION_REPOSITORY;
	...
	const serverUrl = process$2.env.GITHUB_SERVER_URL || "https://github.com";
	const body = { action: action || "" };
	if (serverUrl !== "https://github.com") body.ghes_server = serverUrl;
	try {
		await axios.post(`https://agent.api.stepsecurity.io/v1/github/${process$2.env.GITHUB_REPOSITORY}/actions/maintained-actions-subscription`, body, { timeout: 3e3 });
	} catch (error$2) {
		if (isAxiosError(error$2) && error$2.response?.status === 403) {
